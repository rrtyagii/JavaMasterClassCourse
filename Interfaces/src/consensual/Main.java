 ...
 * 
	//
// Override the toString() method for each of your classes so that they can be easily 
...
// Again, we are not going to use Java file I/O; instead use the readValues() method 
 * below to simulate getting values from a file – this allows you to type as many values as
 *  your class requires, and returns an ArrayList.
// There is a whole Java I/O section later in the course where you will get to use files, etc.
*/
	
	public static void main(String[] args) {
		InterfaceChallenge ic = new InterfaceChallenge("Saw videos", 10); 
		System.out.println(ic.toString());
		saveObject(ic);
		
		ic.setAreYouGonnaDoThisEveryday(false);
		System.out.println(ic);
		ic.setWhatyoudoing("trying to get past the interfaces");
		System.out.println(ic);
		loadObject(ic);
        System.out.println(ic); 
        readValues(); 
	...
	public static void saveObject(ISaveable objectToSave) {
		for(int i=0; i<objectToSave.printArrayList().size(); i++) {
			System.out.println("Saving "+objectToSave.printArrayList().get(i)+" to storage device. ");
		}
	}
	
	public static void loadObject(ISaveable objectToLoad) {
		ArrayList<String> values=new ArrayList<String>() ; 
		objectToLoad.pass(values);
	}
