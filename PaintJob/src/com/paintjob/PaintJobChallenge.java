...
public class PaintJobChallenge {
	/*
 Bob is a wall painter and he needs your help. You have to write a program that helps Bob calculate how many buckets of paint he needs to buy before going to work. Bob might also have some 
...
*/	
   ...
   public static int getBucketCount(double width, double height, double areaPerBucket, int extraBucket) {
	       if(width<=0 || height<=0 || areaPerBucket<=0 || extraBucket<0) {
	    	 return -1 ; 
	       } //  return a value of type int that represents the number of buckets that Bob needs to buy before going to work
	       else {   double BucketCount=0;  
	    	   
		    	     double original=areaPerBucket ; 
		    	     double area= width*height ;
//		    	     System.out.println("The area of the wall is "+area);
		    	     areaPerBucket =Math.ceil((area/original)) ;
		    	     BucketCount=areaPerBucket-extraBucket ; 
   ...
   public static int getBucketCount(double width, double height, double areaPerBucket) {
	   ...
	   else {   double original=areaPerBucket ; 
		        double area=width*height ; 
		        System.out.println("The area of this wall is "+area);
	            areaPerBucket=Math.ceil((area/original)) ; 
...
/*                       
	   ...
	   If all parameters are valid, the method needs to calculate the number of buckets and return it.
*/                else {  
	                double calculatedBuckets=0; 
	                calculatedBuckets=Math.ceil((area/areaPerBucket)) ; 
