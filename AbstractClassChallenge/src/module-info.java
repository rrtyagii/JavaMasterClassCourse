...
// and methods to set the next and previous items.
        //
// You should also specify a compareTo() method that takes a parameter of the same type as 
 * the class and returns 0 if the values are equal, greater than zero if the value sorts
...
// alphabetically). Duplicate values are not added.
        //
// Note that you are creating your own LinkedList class here, not using the built-in Java 
 * one..
        //
 ...
 *  then that is where the new item belongs).
        //
// Care will be needed when inserting before the first item in the list (as it will not 
 * have a previous item).
        //
// You will also need a method to remove an item from the list.
        //
// Hint: If you are creating classes with names such as List, LinkedList, Node etc, make sure
 *that you create your classes before referring to them. In previous videos we have often
 *referred to classes that we create later, but if you use names that IntelliJ recognises
 * as Java classes (such  as LinkedList) then it will create imports for them and 
 *possibly cause you problems later.
        //
// Optional: create a class to use your concrete class to implement a Binary Search Tree:
// When adding items to a Binary Search Tree, if the item to be added is less than the 
 * current item then move to the left, if it is greater than the current item then move to 
 * the right.
        //
